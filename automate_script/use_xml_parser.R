...
args <- read.csv("./user_input.csv", header = F) # COMMENT OUT if hand coding values
...
dates <- read.csv("./Securities.csv") %>%
  ...
  select(date) %>%
  # in case similar auction on same day
  #   e.g. 2-year FRN auction same day as 2-year auction
  #     => puts same date in twice
  distinct()
